...
static NSInteger kMAXPhotoNum = 50;
...
- (void)addPhotoToLibrary
{
    ...
    if (assetIds.count > 0) {
        if (self.sliderModel) {
            ...
            RDAlertAction * action2 = [[RDAlertAction alloc] initWithTitle:@"确定" handler:^{
                ...
                if (weakAlert.textView.text.length == 0) {
                    [MBProgressHUD showTextHUDWithText:@"幻灯片名称不能为空" inView:self.view];
                    return;
                }
        ...
        [MBProgressHUD showTextHUDWithText:@"没有可以添加的照片" inView:self.view];
    }
...
- (void)alertTextFieldDidChange
{
    ...
    if (textFiled.text.length > 0) {
        ...
        if (textFiled.text.length > 20) {
            ...
            [MBProgressHUD showTextHUDWithText:@"最多输入20个字符" inView:self.view];
        }
    }else{
        action.enabled = NO;
    }
        ...
        if (isSelect) {
            if (![weakSelf.selectArray containsObject:asset]) {
                if (weakSelf.selectArray.count + weakSelf.currentNum >= kMAXPhotoNum) {
                    ...
                    [MBProgressHUD showTextHUDWithText:@"最多只能选择50张" inView:self.view];
                    return;
                }
